...
command::Command appCommand(std::vector<desktop::AppEntry> apps,
    ...
    command::Command cmd{"app", "Open an installed app"};
    cmd.search = true;
    ...
    cmd.verbs.push_back({"Open", false, [targets = std::move(targets), launch = std::move(launch),
            history = std::move(history), report = std::move(report)](const auto& args) {
        auto found = targets.find(fuzzy::lower(args.at(0)));
        if (found == targets.end()) return std::string("App unavailable; restart Relay to refresh apps");
        auto error = launch(found->second);
        if (error.empty() && history) {
